...
def delete_user(user_id):
    """
    Endpoint for deleting a user from database by id
    """
    user = DB.get_user_by_id(user_id)
    if user is None:
        return json.dumps({"error": "User not found"}), 404
...
def handle_match():
    """
    Endpoint for a handling one user wanting to match with another
    """
...
def unmatch(match_id):
    """
    Endpoint for deleting a match by its id
    """
    match = DB.get_match_by_id(match_id)
    if match is None:
...
def get_user_matches(user_id):
    """
    Endpoint for getting a user's matches by their user id
    """
    user = DB.get_user_by_id(user_id)
    if user is None:
        return json.dumps({"error": "User not found"}), 404
...
def get_potential_matches(user_id):
    """
    Endpoint for getting a user's potential matches
    """
    user = DB.get_user_by_id(user_id)
    if user is None:
        return json.dumps({"error": "User not found"}), 404
